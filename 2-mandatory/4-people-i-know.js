...
thirtyFiveOrOlder = friends.filter((friends) => friends.age >= 35);
...
let powerNetEmails = [];
powerNetEmails = friends.filter((friends) => friends.company === "POWERNET").map((friends) => friends.email)
/*
...
friendsWhoAreColleaguesOfStacie = friends.filter((friends) => friends.colleagues.some((colleagues) => colleagues.name === "Stacie Villarreal")).map((item) => `${item.name.first} ${item.name.first}`);
...
friends.forEach((friend) => friends.colleagues.map((colleague) => {
