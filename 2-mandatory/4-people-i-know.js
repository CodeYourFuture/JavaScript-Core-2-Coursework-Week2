...
let friendsWhoAreColleaguesOfStacie = friends.filter(friend => friend.colleagues.find(colleaguFriend => colleaguFriend.name ==="Stacie Villarreal" ).map(friend => `${friend.name.first} ${friend.name.last}`));
/*
...
let colleaguesWhoCanMultitask = friends.filter(friend => friend.skills.includes("Multi-tasking").map(friendname =>`${friend.name.first} ${friend.name.last}`));
