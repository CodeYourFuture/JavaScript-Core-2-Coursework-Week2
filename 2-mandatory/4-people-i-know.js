...
let powerNetEmails = [];
friends.filter((friend)=>friend.company === "POWERNET").map((friend) => powerNetEmails.push(friend.email))
/*
...
friends.filter((friend)=>friend.colleagues.some((friendCollege)=>friendCollege.name ==="Stacie Villarreal")).map((friend)=> friendsWhoAreColleaguesOfStacie.push(`${friend.name.first} ${friend.name.last}`))
/* 
