...
//   let obj = weeklyMealPlan[key];
//   for (let prop of obj) {
//     if (!weeklyGroceriesToBuy.includes(prop)) {
//       weeklyGroceriesToBuy.push(prop);
//     }
//   }
// }
...
for (let values of Object.values(weeklyMealPlan)) {
  for (let word of values) {
    if (!weeklyGroceriesToBuy.includes(word)) {
      weeklyGroceriesToBuy.push(word);
    }
  }
}
...
for (let key in weeklyMealPlan) {
  ...
  if (key === "saturday" || key === "sunday") {
    ...
    // console.log(obj);
    for (let prop of obj) {
      if (!weekendGroceriesToBuy.includes(prop)) {
        weekendGroceriesToBuy.push(prop);
      }
    }
  }
}
...
for ([key, value] of Object.entries(weeklyMealPlan)) {
  for ([key1, value1] of Object.entries(numberOfItemsPerWeek)) {
