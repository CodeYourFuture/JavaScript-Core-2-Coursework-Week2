...
function logDeadWritersInTheirForties() {
  ...
  const writersInTheir40sNotAlive = writers.filter((writer) => {
    return writer.age >= 40 && writer.age < 50 && !writer.alive;
  });

  writersInTheir40sNotAlive.forEach((writer) => {
    console.log(
...
function logAliveWritersInTheirForties() {
  ...
  const writersInTheir40sAlive = writers.filter((writer) => {
    return writer.age >= 40 && writer.age < 50 && writer.alive;
  });

  writersInTheir40sAlive.forEach((writer) => {
    console.log(
