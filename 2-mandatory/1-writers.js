...
function logAllWriters() {
  ...
  writers.forEach((write) =>
    console.log(
      `Hi, my name is ${write["firstName"]} ${write["lastName"]}. I am ${write["age"]} years old, and work as a ${write["occupation"]}.`
    )
  );
}
/*
...
function logDeadWritersInTheirForties() {
  ...
  writers.forEach((write) => {
    if (write["age"] >= 40 && write["age"] <= 49 && write["alive"] === false)
      console.log(
        `Writer ${write["firstName"]} ${write["lastName"]} died at ${write["age"]} years old.`
      );
  });
}
