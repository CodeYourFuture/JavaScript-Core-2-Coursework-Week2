...
let friendsWhoAreColleaguesOfStacie = friends.filter(friend => friend.colleagues.some(collegue => collegue.name === "Stacie Villarreal"))
  .map(colleagues => `${colleagues.name.first} ${colleagues.name.last}`);
...
let colleaguesWhoCanMultitask = friends
  .flatMap(friend => friend.colleagues.filter(colleague => colleague.skills.includes("Multi-tasking")))
  .map(colleague => `${colleague.name}`);

  // let colleaguesWhoCanMultitask = friends
  // .filter(friend => friend.colleagues.some(colleague => colleague.skills.includes("Multi-tasking")))
  // .map(friend => friend.colleagues.filter(colleague => colleague.skills.includes("Multi-tasking")))
  // .flat()
  // .map(colleague => `${colleague.name}`);
