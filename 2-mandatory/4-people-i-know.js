...
thirtyFiveOrOlder = friends.filter(friend => friend.age > 34);
...
let powerNetEmails = [];
friends.forEach(friend => friend.company === "POWERNET" ? powerNetEmails.push(friend.email) : null);
...
friends.forEach(friend =>
  friend.colleagues.some(colleague => colleague.name === "Stacie Villarreal") ?
    friendsWhoAreColleaguesOfStacie.push(`${friend.name.first} ${friend.name.last}`) : null);
