...
powerNetEmails = friends
  .filter((friend) => friend.email.includes("powernet"))
...
for (const freind of friends) {
  for (const person of freind.colleagues) {
    if (person.name === "Stacie Villarreal") {
      friendsWhoAreColleaguesOfStacie.push(
        `${freind.name.first} ${freind.name.last}`
      );
    }
  }
}
...
for (const freind of friends) {
  for (const person of freind.colleagues) {
    if (person.skills.includes("Multi-tasking")) {
      colleaguesWhoCanMultitask.push(person.name);
    }
  }
}
