...
for (let day in weeklyMealPlan) {
  let ingredientToBuy = weeklyMealPlan[day].filter(
    (ingredient) => !weeklyGroceriesToBuy.includes(ingredient)
  );
  weeklyGroceriesToBuy = weeklyGroceriesToBuy.concat(ingredientToBuy);
}
