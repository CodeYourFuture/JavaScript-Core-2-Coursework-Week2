...
let thirtyFiveOrOlder = friends.filter((friends) => friends.age >= 35)
...
for (let value of friends) {
  if (value.company === 'POWERNET') {
    powerNetEmails.push(value.email);
  }
}
...
for (let firstArrElem = 0; firstArrElem < friends.length; firstArrElem++) {
  for (let secondArrElem of friends[firstArrElem].colleagues) {
    if (secondArrElem.name === 'Stacie Villarreal') {
      friendsWhoAreColleaguesOfStacie.push(`${friends[firstArrElem].name.first} ${friends[firstArrElem].name.last}`)
    }
  }
}
...
for (let firstArrElem = 0; firstArrElem < friends.length; firstArrElem++) {
  for (let secondArrElem of friends[firstArrElem].colleagues) {
    if (secondArrElem.skills.includes("Multi-tasking")) {
      colleaguesWhoCanMultitask.push(secondArrElem.name)
    }
  }
}
