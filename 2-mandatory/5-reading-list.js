...
const books = [
  ...
  { title: "The Map of Salt and Stars", author: "Jennifer Zeynab Joukhadar", alreadyRead: true },
  { title: "Dietland", author: "Sarai Walker", alreadyRead: true },
  { title: "A Place for Us", author: "Fatima Farheen Mirza", alreadyRead: false },
  { title: "The House of Impossible Beauties", author: "Joseph Cassara", alreadyRead: true },
];

// exercise 1
const logBooks = () => books.forEach(({ title, author }) => console.log(`${title} by ${author}`));

const logBooksByReadStatus = () => books.forEach((book) => console.log(`${book.alreadyRead ? "You've already read" : "You still need to read"} ${book.title} by ${book.author}`));
...
test("Books are logged by read/unread", () => {
  expectLogBooksToLogByRead([
...
function expectLogBooksToLogByRead(expectedValues) {
  const consoleLogSpy = jest.spyOn(console, "log");
  logBooksByReadStatus();
  expect(consoleLogSpy).toBeCalledTimes(expectedValues.length);
  expectedValues.forEach((value, i) => {
    expect(consoleLogSpy).nthCalledWith(i + 1, value);
  });
