...
let thirtyFiveOrOlder = friends.filter(function (friend) {
  if (friend.age >= 35) {
...
let powerNetEmails = [];
friends
  .filter((eml) => eml.email)
  .reverse()
  .filter(function (friendCompany) {
    if (friendCompany.company === "POWERNET") {
...
let friendsWhoAreColleaguesOfStacie = friends
  .filter((colleagueAll) =>
    colleagueAll.colleagues.some(
      (colleagueName) => colleagueName.name === "Stacie Villarreal"
    )
  )
