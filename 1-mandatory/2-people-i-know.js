...
let friendsWhoAreColleaguesOfStacie = friends
    .filter(friend => (friend.colleagues
        .some(colleague => colleague.name === "Stacie Villarreal")))
...
let colleaguesWhoCanMultitask = friends.reduce((acc, friend) => {
    let colleagesWithSkill = friend.colleagues.filter(col => col.skills.includes("Multi-tasking")).map(col => col.name);
    return [...acc, ...colleagesWithSkill]
},[]);
