...
for (const day in weeklyMealPlan) {
  for (const groceryItem of weeklyMealPlan[day]) {
    if (weeklyGroceriesToBuy.indexOf(groceryItem) === -1) {
...
for (const key in weeklyMealPlan) {
  if (key === "saturday" || key === "sunday") {
    for (const groceryItem of weeklyMealPlan[key]) {
      if (weekendGroceriesToBuy.indexOf(groceryItem) === -1) {
        weekendGroceriesToBuy.push(groceryItem);
      }
    }
  }
}
