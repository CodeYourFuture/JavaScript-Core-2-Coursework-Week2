...
for (let day in weeklyMealPlan) {
  for (let item in weeklyMealPlan[day])
    if (!weekendGroceriesToBuy.includes(weeklyMealPlan[day][item]))
      weeklyGroceriesToBuy.push(weeklyMealPlan[day][item]);
}
/*
Exercise 2:
...
for (let day in weeklyMealPlan) {
  if (day === saturday || day === sunday)
    for (let item in weeklyMealPlan[day])
      if (!weekendGroceriesToBuy.includes(weeklyMealPlan[day][item]))
        weekendGroceriesToBuy.push(weeklyMealPlan[day][item]);
}
