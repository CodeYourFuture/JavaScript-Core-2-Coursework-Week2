...
friends.forEach((person) => {
  if (person.company.toLowerCase() === "powernet") {
...
friends.forEach((person) => {
  let colleaguesOfStacie = person.colleagues.find(
    (colleague) => colleague.name.toLowerCase() === "stacie villarreal"
  );
  if (colleaguesOfStacie) {
...
friends.forEach((person) => {
  person.colleagues.forEach((colleague) => {
    let multitask = colleague.skills.find(
      (skill) => skill.toLowerCase() === "multi-tasking"
    );
    if (multitask) {
