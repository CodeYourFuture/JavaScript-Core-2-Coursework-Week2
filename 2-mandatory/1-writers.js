...
function logAllWriters() {
  for (let writer of writers) {
    let writerfirstname = writer.firstName;
    let writerlastname = writer.lastName;
    let writerage = writer.age;
    let job = writer.occupation;
    console.log(
      `Hi , my name is ${writerfirstname} ${writerlastname}. i am ${writerage} years old , and work as a ${job} `
    );
  }
}
...
function logDeadWritersInTheirForties() {
  for (let writer of writers) {
    let writerAge = writer.age;
    if (writerAge > 39 && writerAge < 50 && writer.alive === false)
      console.log(
...
function logAliveWritersInTheirForties() {
  for (let writer of writers) {
    let writerAge = writer.age;
    if (writerAge > 39 && writerAge < 50 && writer.alive)
      console.log(
