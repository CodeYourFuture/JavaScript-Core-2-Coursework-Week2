...
function logDeadWritersInTheirForties() {
  for (let x in writers) {
    if (
      writers[x].age >= 40 &&
      writers[x].age <= 50 &&
      writers[x].alive === false
    ) {
      console.log(
        `Writer ${writers[x].firstName} ${writers[x].lastName} died at ${writers[x].age} years old.`
      );
    }
  }
}
...
function logAliveWritersInTheirForties() {
  for (let x in writers) {
    if (
      writers[x].age >= 40 &&
      writers[x].age <= 49 &&
      writers[x].alive === true
    ) {
      console.log(
        `Writer ${writers[x].firstName} ${writers[x].lastName}. I am ${writers[x].age} years old.`
      );
    }
  }
}
