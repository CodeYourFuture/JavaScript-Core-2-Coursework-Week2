...
function chooseMeal(mealsArr) {
  let stringVal = "Nothing :(";
  if (mealsArr.length > 1) {
    ({ name: stringVal } = mealsArr.sort((a, b) => a.price - b.price)[1]);
  } else if (mealsArr.length === 1) {
    stringVal = mealsArr[0]["name"] ? mealsArr[0]["name"] : stringVal;
  }
