...
thirtyFiveOrOlder = friends.filter((obj) => obj.age >= 35);
...
let powerNetEmails = [];
friends.filter((obj) => {
  if (obj.company === "POWERNET") {
    console.log(obj.email);
    powerNetEmails.push(obj.email);
  }
});
...
friends.filter((obj) => {
  obj.colleagues.filter((nestedObj) => {
    if (nestedObj.skills.includes("Multi-tasking")) {
      colleaguesWhoCanMultitask.push(nestedObj.name);
    }
  });
});
