...
friends.forEach(element => {
  // element.age >= 35 ? thirtyFiveOrOlder.push(element.name.first) : null;
  element.age >= 35 ? thirtyFiveOrOlder.push(element) : null;
});
...
let powerNetEmails = [];
friends.forEach(element => {
  element.company === "POWERNET" ? powerNetEmails.push(element.email) : null;
});
...
friends.forEach(element => {
  for (const iterator of element.colleagues) {
    iterator.name === "Stacie Villarreal"
...
friends.forEach(e => {
  for (const iterator of e.colleagues) {
    iterator.skills.includes("Multi-tasking")
      ? colleaguesWhoCanMultitask.push(iterator.name)
      : null;
  }
});
