...
for (let list in weeklyMealPlan) {
  weeklyMealPlan[list].forEach((item) => {
...
Object.entries(weeklyMealPlan).forEach(([key, value]) => {
  if (key === "saturday" || key === "sunday") {
    value.forEach((item) => weekendGroceriesToBuy.push(item));
  }
});
...
for (let weekDay in weeklyMealPlan) {
  let ingredients = weeklyMealPlan[weekDay].length;
  numberOfItemsPerWeek[weekDay] = ingredients;
}
