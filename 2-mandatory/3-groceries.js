...
for (products in weeklyMealPlan){
  weeklyGroceriesToBuy.push(weeklyMealPlan[products]);
}
weeklyGroceriesToBuy = Array.from(
  new Set(weeklyGroceriesToBuy.flatMap((x)=> x))
);
/*
Exercise 2:
...
weekendGroceriesToBuy = Array.from(
  new Set(weekendGroceriesToBuy.flatMap((x) => x))
);
/*
Exercise 3:
