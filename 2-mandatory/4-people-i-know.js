...
////////// SOLUTION 2 /////////
// let powerNetEmails =[];
// powerNetEmails =  friends.filter((friend) => {
//   if(friend.company === 'POWERNET'){
//     return friend;
//   }
// }).map((friend) => {
...
///////// SOLUTION 3///////
let powerNetEmails = [];
 powerNetEmails = friends.filter((friend) => friend.company === 'POWERNET')
...
friendsWhoAreColleaguesOfStacie = friends.filter((friend) => friend.colleagues
.some((college) => college.name === "Stacie Villarreal"))
...
for( let friendArray of friends){
  for(let colleagueArray of friendArray.colleagues){
    if(colleagueArray.skills.includes('Multi-tasking')){
      colleaguesWhoCanMultitask.push(colleagueArray.name)
    }
  }
}
