...
friends.forEach((friend)=>{
  let isColleage = friend.colleagues
    .map((colleagueName) => colleagueName.name)
...
friends.map((friend)=>friend.colleagues)
.forEach((items)=>{
  for(let key in items){
    if(items[key].skills.includes("Multi-tasking")){
      colleaguesWhoCanMultitask.push(items[key].name);
    }
  }
})
