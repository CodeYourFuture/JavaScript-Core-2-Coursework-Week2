...
let thirtyFiveOrOlder = friends.reduce((acc, val) => {
  if (val.age >= 35) acc.push(val);
  return acc;
}, []);
/*
...
let powerNetEmails = friends.reduce((acc, val) => {
  if (val.company === "POWERNET") acc.push(val.email);
  return acc;
}, []);
/*
