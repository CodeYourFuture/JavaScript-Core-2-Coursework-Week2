...
thirtyFiveOrOlder = friends.filter((old) => old.age >= 35); 
...
let powerNetEmails = [];
let powerNetFriends = friends.filter((friend) => friend.company === "POWERNET"); 
powerNetEmails = powerNetFriends.map((friend) => friend.email); 
...
let colleagueOfStacie = friends.map((friend) => friend.colleagues.map((colleague) => {
...
let usefulColleagues = friends.map((friend) => friend.colleagues).flat(); 
usefulColleagues.map((colleague) => {
