...
friends.forEach(function (friend) {
  ...
  try {
    for (var _iterator = friend.colleagues[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      var colleague = _step.value;
...
test("2 - friends that are over 35", function () {
  expect(thirtyFiveOrOlder.map(function (_ref) {
    var name = _ref.name;
    return name.first;
  })).toIncludeSameMembers(["Vilma", "Aisha", "Mitchell", "Sutton", "Jana"]);
});
test("3 - Powernet email addresses", function () {
  expect(powerNetEmails).toIncludeSameMembers(["clay.livingston@powernet.com", "gloria.hall@powernet.com"]);
});
test("4 - friends with Stacie Villarreal as a colleague", function () {
  expect(friendsWhoAreColleaguesOfStacie).toIncludeSameMembers(["Clay Livingston", "Jana Harrison", "Haley Knox"]);
});
test("5 - colleagues who can multitask", function () {
  expect(colleaguesWhoCanMultitask).toIncludeSameMembers(["Rush May", "Gena Good", "Cunningham Shelton", "Castro Castaneda", "Luz Newton"]);
});