...
let friendsWhoAreColleaguesOfStacie = friends.filter(friend => friend.colleagues.find(colleague => colleague.name === "Stacie Villarreal")).map(friend => `${friend.name.first} ${friend.name.last}`);
/*
...
// let colleaguesWhoCanMultitask = friends.filter(friend => friend.colleagues.filter(colleague => colleague.skills.includes("Multi-tasking")))


// let colleaguesWhoCanMultitask = friends.filter(friend => friend.colleagues.filter(colleague => {
//    if (colleague.skills.includes("Multi-tasking")) return colleague.name
// }))
...
.map(colleague => colleague.name)).filter(friend => friend.length > 0);

for( item of myArr) {
if (item.length > 1){
    for(let j = 0; j < item.length; j++) {
        colleaguesWhoCanMultitask.push( item[j])
    }
   } else  {colleaguesWhoCanMultitask.push(item.join(""));
   }
  }
  //really not the best way but running out of time
